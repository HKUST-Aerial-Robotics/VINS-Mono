...
public:
	KeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, cv::Mat &_image,
			 vector<cv::Point3f> &_point_3d, vector<cv::Point2f> &_point_2d_uv, vector<cv::Point2f> &_point_2d_normal, int _sequence);
	...
	void searchByBRIEFDes(std::vector<cv::Point2f> &matched_2d_cur,
							   std::vector<cv::Point2f> &matched_2d_old,
	                           std::vector<cv::Point3f> &matched_3d,
	                           const std::vector<BRIEF::bitset> &descriptors_old,
	                           const std::vector<cv::KeyPoint> &keypoints_old);
	void FundmantalMatrixRANSAC(std::vector<cv::Point2f> &matched_2d_cur,
	                                      std::vector<cv::Point2f> &matched_2d_old,
	                                      std::vector<cv::Point2f> &matched_2d_cur_norm,
	                                      std::vector<cv::Point2f> &matched_2d_old_norm,
	                                      std::vector<cv::Point3f> &matched_3d);
	void PnPRANSAC(vector<cv::Point2f> &matched_2d_cur,
