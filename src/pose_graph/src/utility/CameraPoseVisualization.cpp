...
void CameraPoseVisualization::add_loopedge(const Eigen::Vector3d& p0, const Eigen::Vector3d& p1){
    ...
    marker.action = markerMsg::ADD;
    marker.lifetime = rclcpp::Duration(0, 400000);
    //marker.scale.x = 0.4;
    marker.scale.x = 0.02;
    marker.color.r = 1.0f;
    //marker.color.g = 1.0f;
...
void CameraPoseVisualization::publish_by(rclcpp::Publisher<markerArrayMsg>::SharedPtr pub,
                                                     const std_msgs::msg::Header &header ) {
	markerArrayMsg markerArray_msg;
...
void CameraPoseVisualization::publish_image_by(rclcpp::Publisher<markerMsg>::SharedPtr pub, 
                                                            const std_msgs::msg::Header &header) {
    image.header = header;
    pub->publish(image);
}
